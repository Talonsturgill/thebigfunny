"""VO for Dispatch 'Sorted By A Machine' (an LLM screened NEH grants for 'DEI', flagging an Alaska
Native Language Archive grant among ~1,400 terminated; a court ruled it unlawful, reinstatement began
June 2026). Fresh warm female voice (edge-tts en-US-Ava) via the agent proxy; kokoro fallback kept.
13 phrases mapped to 6 shots (segments 0-5). Writes vo60.wav, timing60.json, words60.json,
voice_used.json into the skill audio dir AND a mirror in out/dispatch/audio.
No em/en dashes anywhere. Numbers written for the synth. Facts fact-checked (IHE/AHA/UAF Sun Star).
English-only on screen (cultural respect: no Native words/orthography rendered).
"""
...
KOKORO_VOICE = "af_heart"; KOKORO_SPEED = 0.92
EDGE_VOICE = "en-US-AvaMultilingualNeural"; EDGE_RATE = "+2%"
...
PHRASES = [
    ("Not long ago, more than a thousand research grants were sorted by a machine.", 0),
    ("The question it was asked was simple. Does this relate, at all, to D.E.I.?", 0),
    ("Each proposal went to a chatbot, which answered in a single word.", 1),
    ("Yes, or no.", 1),
    ("One of them was a plan to save the Alaska Native Language Archive.", 2),
    ("Decades of elders' voices, the last fluent recordings of whole languages.", 2),
    ("The chatbot answered yes, it relates.", 3),
    ("So the grant was terminated, along with about fourteen hundred others.", 3),
    ("A hundred million dollars, gone in twenty-two days.", 3),
    ("But the chatbot did not decide. A person aimed it, and a person signed.", 4),
    ("Then a court ruled the cuts unlawful, and this summer the funding was offered back.", 4),
    ("Offered back is not restored. The lost year does not return.", 5),
    ("An archive of a language is memory. It deserves a human reading every line.", 5),
]
...
def kokoro_synth(text, idx):
    ...
    if KP is None:
        ...
        KP = KPipeline(lang_code="a")   # 'a' = American English
...
try:
    ...
    voice_lic = "Microsoft Azure Neural TTS (draft cut; gray area for commercial publish, re-cut in Kokoro for publish)"
