...
> Alaska fauna, biomes and props. They are REAL and usable; a moose is funny.
>
> What is NOT here: the upstream `docs/craft/*` reference docs, which were not
> ported. Entries that used to cite them now say so instead of pointing at
> nothing, because `scripts/refs_check.py` treats a dangling pointer as a defect:
> a run told to read a missing doc does not error, it proceeds and answers
> confidently from nothing.
>
> THE FIXED CAST (Ray, Dee, the Institution) is the section immediately below
...
- `StairwellBG` — NET-NEW 2026-08-02 (case 0003, `the-landlord` beat) — lib/biomes.tsx: the shelf's first HOUSING interior. An apartment stairwell landing: putty-green walls, an oxblood panelled door with a numbered tag, brass rail and posts on two flights climbing away, a skylight shaft with hard light bars falling DOWN it and dust drifting in them, flat tiled landing floor. Params `light` 0..1, `doorNo` (the edge-tease). Deliberately VERTICAL: case 0001 was a flat interior wall and case 0002 a horizontal one-point recession, so a corridor of receding doors would have been 0002 in a hat; here the recession goes up and the cast stays near camera and large, which matters now that faces carry the performance. Stairs are a stepped top over a DIAGONAL soffit — pass 1 closed them with a flat bottom and they read as a jagged mountain, caught by rendering a still. THE housing/eviction/rent stage — ACTIVE
