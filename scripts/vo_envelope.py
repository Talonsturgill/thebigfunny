...
def envelope(samples, sr, fps=FPS):
    """RMS per video frame -> gated, normalised, asymmetrically smoothed 0..1."""
    ...
    for v in rms:
        ...
        out.append(round(min(1.0, max(0.0, prev)) * MAX_OPEN, 3))
    return out


def accents(env):
    """Per-frame ONSET strength 0..1: where the voice pushes, so the BODY can hit it.

    This is the half that makes a figure look like it is speaking rather than
    standing near some audio. The 12 principles call it accent and secondary
    action: a gesture lands ON the stressed syllable and the body is otherwise
    still. The rig it feeds used to move on free-running sines (bob sin(f/12),
    sway sin(f/88), mouth sin(f/4.2)), none of which knew a word was happening,
    and continuous motion unrelated to speech is read as drift, not performance.
...
def render_ts(env, case, name):
    acc = accents(env)
    return (HEADER.format(name=name, case=case, fps=FPS)
            + "export const MOUTH: number[] = [\n" + _rows(env) + "\n];\n\n"
            + "/** Where the voice PUSHES, 0..1, for the body to hit. See accents(). */\n"
            ...
            + "/** Onset strength at a global frame, for gesture accents. */\n"
...
def self_test():
    ...
    checks += [
        ...
        ("silence produces no accent", max(accents(quiet)) == 0.0),
    ]
...
def main():
    ...
    if not env:
        ...
        return 1
    text = render_ts(env, a.case, name)
